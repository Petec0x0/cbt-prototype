...
$query = mysqli_query($link, "select id_question, question_name, question_image, option1, option2, option3, option4, answer from tb_question where 0;");

// Processing form data when form is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $exam_id = trim($_POST["exam_id"]);

    $query = mysqli_query($link, "select id_question, question_name, question_image, option1, option2, option3, option4, answer from tb_question where group_id = $exam_id;");
}
    ...
    <div class="wrapping">
        ...
        <div class="content-wrapping" style="text-align: justify;">
            ...
            <?php if (isset($_GET['msg'])) {
                if ($_GET['msg'] === "success") {
                    echo "<div class='alert alert-success' role='alert'>Successfully saved changes!</div>";
                } else {
            ...
            <table id="viewResult" class="table display" style="width:100%">
                ...
                <tbody>
                    ...
                    while ($view = mysqli_fetch_assoc($query)) {
                        ...
                        echo "<td>";
                        if ($view['question_image'] !== "" && !is_null($view['question_image'])) {
                            echo "<img src='../$view[question_image]' style='max-width: 75%'><br>";
                        }
                        echo $view['question_name'];
                        echo "</td>";

                        echo "<td>" . $view['option1'] . "</td>";
                        echo "<td>" . $view['option2'] . "</td>";
                        echo "<td>" . $view['option3'] . "</td>";
                        echo "<td>" . $view['option4'] . "</td>";
                        echo "<td>" . $view['answer'] . "</td>";
                        echo "<td> <a href=\"editquestion.php?id=$view[id_question]\">Edit</a> </td>";
                        echo "<td> <a href=\"deletequestion.php?id=$view[id_question]\" onClick=\"return confirm('Anda yakin ingin menghapus?')\">Delete</a> </td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
