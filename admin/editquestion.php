...
$query = mysqli_query( $link, "select question_name, question_image, option1, option2, option3, option4, answer from tb_question where id_question='$id_question';");
$value = mysqli_fetch_assoc($query);

// Define variables and initialize with empty values
$question_name = $option1 = $option2 = $option3 = $option4 = $answer = $question_image = "";
$question_err = $option1_err = $option2_err = $option3_err = $option4_err = $answer_err = $image_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($question_err) && empty($option1_err) && empty($option2_err) && empty($option3_err) && empty($option4_err) && empty($answer_err) && empty($image_err) ){
        
        // Prepare an insert statement
        $sql = "UPDATE tb_question set question_name=?, question_image=?, option1=?, option2=?, option3=?, option4=?, answer=? where id_question=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssii", $param_question, $param_image, $param_option1, $param_option2, $param_option3, $param_option4, $param_answer, $param_id);
...
@media (max-width: 992px) {
  ...
  <body style="font-family: 'Roboto', sans-serif">
    ...
    <div class="wrapping">
      ...
      <div class="container">
          <center><h2>Change Question</h2></center>
          <br>
    ...
    <div class="form-group">
                ...
                <select class="custom-select <?php echo (!empty($answer_err)) ? 'is-invalid' : ''; ?>" name="answer">
                  ...
                  <option value="1" <?php if($value['answer'] == 1){ echo "selected";} ?>>Option 1</option>
                  <option value="2" <?php if($value['answer'] == 2){ echo "selected";} ?>>Option 2</option>
                  <option value="3" <?php if($value['answer'] == 3){ echo "selected";} ?>>Option 3</option>
                  <option value="4" <?php if($value['answer'] == 4){ echo "selected";} ?>>Option 4</option>
                </select>
