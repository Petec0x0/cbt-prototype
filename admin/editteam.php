...
$query = mysqli_query( $link, "select id_team, name_team from tb_team where id_team='$id_team';");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST['nameteam']))){
        $nameteam_err = "Please enter nama tim.";     
    } else{
    ...
    if(empty($nameteam_err)){
        ...
        $sql = "UPDATE tb_team set name_team=? WHERE id_team=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "si", $param_nameteam, $param_idteam);
...
@media (max-width: 992px) {
  ...
  <body style="font-family: 'Roboto', sans-serif">
    ...
    <div class="wrapping">
        ...
        <div class="container">
	     <center><h2>Change Exam Group</h2></center><br>
    ...
    <input type="text" class="form-control" value="<?php echo $value['id_team']; ?>" name = "idteam" readonly hidden/>

    <div class="form-group">
      <label for="nameteam">Change Name Exam Group</label>
      <div class="input-group mb-3"><div class="input-group-prepend">
        <span class="input-group-text"><i class="fa fa-handshake-o"></i></span>
        </div>
        <input type="text" class="form-control <?php echo (!empty($nameteam_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $value['name_team']; ?>" name = "nameteam" placeholder="Name Exam Group" />
      </div>
