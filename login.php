...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($username_err) && empty($password_err)) {
    ...
    $sql = "SELECT username, nama, password, done, name_team FROM tb_user JOIN tb_team ON team=id_team WHERE username = ?";
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
          ...
          mysqli_stmt_bind_result($stmt, $username, $nama, $hashed_password, $done, $team);
          if (mysqli_stmt_fetch($stmt)) {
            if (password_verify($password, $hashed_password)) {
              ...
              $_SESSION['done'] = 0;
              $_SESSION['team'] = $team;
              header("location: antecedent.php");
              // if ($done == 0) {

              // } else {
              //   // Display an error message if user has written exam
              //   $error_msg = 'User has witten this exam';
              // }
            } else {
    ...
    <div class="card">
      ...
      <div class="card-body">
        <form id="login" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
          ...
          <center>Please <a href="register.php">click here</a> if not registered.</center>
        </form>
      </div>
