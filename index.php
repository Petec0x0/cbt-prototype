  ...
  <div class="card">
    ...
    <div id="question_body" class="card-body" style="font-size: 11pt; text-align: justify;">
      <form class="form-horizontal" role="form" method="post" action="result.php">
          ...
          <!-- Opened -->
          <span id="textsetting-opened" style="font-size: 14pt; text-align: left;">Text Settings
...
<input type="radio" checked='checked' style='display:none' value="0" id='radio1_<?php echo $result['id_question'];?>' name='<?php echo $result['id_question'];?>'/>
            ...
            <?php
               ...
               if ( ( $remainder < 1 ) || ( $i == $question_per_page && $remainder == 1 ) ) {
                echo "<br><br><button id='".$j."' class='btn btn-outline-primary' type='submit' onClick=\"return confirm('Are you sure you want to complete the quiz?');\">Finished</button>";
                echo "</div>";
               }  elseif ( $rowcount > $question_per_page  ) {
                if ( $j == 1 && $i == $question_per_page ) {
                  echo "<button id='".$j."' class='next btn btn-outline-secondary' type='button'>Next</button>";
                  echo "</div>";
                  $i = 0;
                  $j++;           
                } elseif ( $k == $rowcount ) { 
                  echo "<button id='".$j."' class='previous btn btn-outline-secondary' type='button'>Previous</button>
                        <br><br><button id='".$j."' class='btn btn-outline-primary' type='submit' onClick=\"return confirm('Are you sure you want to complete the quiz?');\">Finished</button>";
                  echo "</div>";
                  $i = 0;
                  $j++;
                } elseif ( $j > 1 && $i == $question_per_page ) {
                  echo "<button id='".$j."' class='previous btn btn-outline-secondary' type='button'>Previous</button>
                                  <button id='".$j."' class='next btn btn-outline-secondary' type='button' >Next</button>";
                  echo "</div>";
                  $i = 0;
                  $j++;
                }
...
var x = setInterval(function() {
    ...
    // document.getElementById("timer").innerHTML = days_remaining + " Hari " + hours_remaining + " Hour "
    // + minutes_remaining + " Minutes " + seconds_remaining + " Seconds ";
    
    document.getElementById("timer").innerHTML = hours_remaining + " Hour "
    + minutes_remaining + " Minutes " + seconds_remaining + " Seconds ";
